...
function updatePositions() {
  ...
  // Moved the code that queries the document to find all of the pizza objects into
  // the addEventListener() function. No need to run that query with every time the
  // page is scrolled.

  // Moved the code that queries the current scrollTop element outside of the for loop
  var scrollTopTmp = document.body.scrollTop / 1250;
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin((scrollTopTmp) + (i % 5));
...
// Create variable items, which will be populated with all of the pizza when the
// page is first loaded (no need to query to document repeatedly to get this info,
// since it doesn't change once the page is loaded).
var items;
